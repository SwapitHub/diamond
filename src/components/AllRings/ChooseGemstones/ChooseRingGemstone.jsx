...
export const ChooseRingGemstone = () => {
  ...
  // console.log(filterData.product?.similar_products);
  useEffect(() => {
  ...
  // function handleWishlist(item, data) {
  ...
  //     ring_color: listColor,
  //     stock_num: diamondData.stock_num,
  //     gemstone: diamondData,
  //     totalSalesPrice: diamondData.total_sales_price,
  //     uniqueId: uuidv4(),
  //   };
  ...
  const handleWishlist = async (
    ...
    try {
      const diamondData = Object.assign({}, ...newData);
      const removingItem = {
        ringPrice: ringPrice,
        product_id: filterData.product?.id,
        ring_data: filterData.product,
      ...
      if (response.status === 200) {
        console.log("Wishlist item added successfully:", response.data.data.id);
  ...
  useEffect(() => {
    ...
    const debouncedFetchData = debounce(fetchData, 200); // Adjust delay as needed (in milliseconds)
  ...
  const handleVariation = (variantSlug) => {
    ...
    const newURL = `${"/detail-ring-product-gemstone"}?${newSearchString}`;
    console.log(newURL);
  ...
  return (
    <div className="details-page container">
      <div className="bread-cram-setting-pages">
        <Link to="#">Home</Link> / <Link to="#">Engagement Rings</Link> /{" "}
        <Link to="#">Ring style </Link> / Ring Name
      </div>
            ...
            {!shapeData ? (
              ...
              <>
                <div className="singleProduct-img">
                  <div className="choose-gemstones details-videos">
                    <LazyLoadImage
